...
export default function Sidebar() {
  ...
  return (
    <aside className="app-sidebar" style={{ 
      ...
      {/* Menu */}
      <nav className="menu" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
        {items.map((item) => {
      ...
      <div style={{
        ...
        <Link
          ...
          onClick={logout}
          style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 16px', borderRadius: 10,
            color: 'var(--alert-red)', fontSize: '0.88rem', textDecoration: 'none',
      ...
      >
        <div style={{ width: 42, height: 42, borderRadius: 12, background: 'var(--primary)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '1rem', boxShadow: '0 4px 10px rgba(16,185,129,0.3)' }}>
          {user?.initials || 'MC'}
        </div>
