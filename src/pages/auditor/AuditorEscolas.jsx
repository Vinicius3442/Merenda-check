        ...
        {mockEscolas.map((escola) => (
          <div key={escola.id} className={`glass-panel school-card ${escola.healthClass}`} style={escola.health < 70 ? { borderWidth: 2 } : {}}>
            ...
            <div className="card-actions">
              {escola.health < 70 ? (
                <>
                  <Link to="/auditor/rastrear" className="btn btn-primary" style={{ flex: 1, padding: 10, fontSize: '0.8rem' }}>Ler Blackbox</Link>
                  <button className="btn btn-secondary" style={{ flex: 1, padding: 10, fontSize: '0.8rem' }} onClick={() => mockSubmit({ successTitle: 'Auditoria Expedida', successMsg: 'Aviso extrajudicial enviado para E-mail da gestão via portaria eletrônica.' })} disabled={loading}>
                    Notificar Gestor
                  </button>
                </>
              ) : escola.health < 95 ? (
                <button className="btn btn-secondary" style={{ width: '100%', padding: 10, fontSize: '0.8rem' }} onClick={() => mockSubmit({ successMsg: 'Forçando remanejamento automático de excesso para EMEF João Silva.' })} disabled={loading}>
                  <i className="fa-solid fa-truck-arrow-right"></i> Sugerir Remanejamento IA
                </button>
              ) : (
                <button className="btn btn-secondary" style={{ width: '100%', padding: 10, fontSize: '0.8rem' }} onClick={() => mockSubmit({ successMsg: 'Visualizando log de eventos de 7 dias...' })} disabled={loading}>
                  <i className="fa-regular fa-eye"></i> Visualizar Oráculo Normal
                </button>
              )}
            </div>
          </div>
